...
class Telnet:
    ...
    def msg(self, msg, *args):
        ...
        if self.debuglevel > 0:
            idstr = 'telnetlib(%s,%s): {msg}' % (self.host, self.port)
            if args:
                msg = str(msg % args)
            else:
                msg = str(msg)

            self.log( idstr.format(msg=msg) )

    def set_debuglevel(self, debuglevel, log = lambda x:x):
        """Set the debug level.
